...
export default function Parrainage() {
  ...
  const [error, setError] = useState(false);
  const [success, setSuccess] = useState(false);
  const [infosParrainage, setInfosParrainage] = useState(null);
  const [candidats, setCandidats] = useState([]);

  useEffect(() => {
    // Assurez-vous que localStorage est défini avant d'accéder à ses propriétés
    if (typeof window !== 'undefined') {
      const parrainageInfo = localStorage.getItem("infosParrainage");
      if (parrainageInfo) {
        setInfosParrainage(JSON.parse(parrainageInfo));
        setCandidats(JSON.parse(parrainageInfo)?.candidats || []);
      } else {
        // Gérer le cas où infosParrainage est null
        router.push("/");
      }
    }
  }, []);
  ...
  const handleParrainage = async (candidat) => {
    ...
    try {
      const response = success
        ? await validerParrainage({ numElecteur: infosParrainage.numElecteur, code5: inputValues.code5 }, selectedCandidat.ID)
        : await envoiCodeValidation({ numElecteur: infosParrainage.numElecteur, codeAuth: inputValues.codeAuth }, selectedCandidat.ID);
